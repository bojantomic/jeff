...
/**
 * A concrete ReportBuilder implementation. This class is used when the output
 * report is supposed to be JSON.
 *
...
public class JSONReportBuilder extends ReportBuilder {
	 ...
	 * 
	 * Basically, this method opens the java.io.FileWriter output stream and it
	 * creates com.google.gson.JsonObject object (based on the provided file
	 * path) which holds all of the explanation information before it is put
	 * into the stream, and calls the "buildReport" method that has an
	 * com.google.gson.JsonObject an argument.
	 *
	 * @param explanation
	 *            the explanation that needs to be transformed into a report
	 * @param filepath
	 *            a string representing an URL for the file
	 * @throws org.goodoldai.jeff.explanation.ExplanationException
	 *             if any of the arguments are null, if filepath is an empty
	 *             string, or if IOException is caught
	 */
	@Override
	 ...
	 * provided object that is type of com.google.gson.JsonObject before it is
	 * written in the file
	 *
	 * @param explanation
	 *            the explanation that needs to be transformed into a report
	...
	public void buildReport(Explanation explanation, Object stream) {
		...
		//creating JSON object to represent explanation 
		//to represent whole explanation to be written in JSON file
		JsonObject object = new JsonObject();
		insertHeader(explanation, object);
		//adding explanation array to single out explanation header from explanation chunks
		...
		if (writer.checkError()) {
			throw new ExplanationException("The file could not be writen due to connection problems with PrintWriter.");
		}
	}
	 ...
	 * owner, title, language and country). If any of this data is missing, it
	 * is not inserted into the report. Since the report format is JSON, the
	 * provided output stream should be an instance of
	 * com.google.gson.JsonObject.
	 * 
	 * @param explanation
	 *            explanation from which the header data is to be collected
	 * @param stream
	 *            JsonObject that the header is supposed to be inserted into
	 * 
	 * @throws org.goodoldai.jeff.explanation.ExplanationException
	 *             if any of the arguments are null or if the entered output
